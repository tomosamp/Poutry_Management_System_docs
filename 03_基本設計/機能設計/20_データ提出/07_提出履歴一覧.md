...
## 1. 目的
- 提出月（締切月）を基準に対象月（実績月）の提出状況（未提出/提出済/変更あり）を一覧で把握し、詳細確認・再提出へ素早く遷移できるようにする。

## 2. 用語
- 提出月（締切月）: 画面入力で選択する月。本一覧の基準月とする。
- 対象月（実績月）: 提出月に対して `提出月 - 1ヶ月` を指す。DBでは `monthly_records.target_month`（月初日）として保持する。
...
## 4. 入力（クエリ）
- `submission_month`（YYYY-MM）
  - 未指定時は運用ルール（提出期限=毎月15日）に従い、当月の提出月を既定値として表示する（`SubmissionPeriod` の `deadline`）。
...
## 6. 判定ロジック（対象月の最新実績）
1. 対象月（`target_month`）について、施設ごとに `monthly_records`（`data_kind=actual`）の最新レコードを特定する（対象月は `submission_month - 1ヶ月`）。
