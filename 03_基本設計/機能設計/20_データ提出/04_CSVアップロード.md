...
4. 入力確認画面へ遷移し、利用者が必要なら値を修正して確定登録する。

## 生産者（施設）割り出しロジック（確定）
- ブロイラー: C〜H（地区列）のうち、最初に値が入っているセルから `^[a-f][0-9]+｜` を除去した文字列を「生産者名キー」とする。
- 地鶏: C〜H（地区列）のうち、最初に値が入っているセルから `^[a-f][0-9]+｜` を除去した文字列を「生産者名キー」とする。
- 成鶏: C（企業・団体名）の文字列を「生産者名キー」とする（地区列なし）。
- 照合: 鳥種で `facilities` を絞り、以下の正規化後に一致で特定する。
  - 前後の空白（全角/半角）除去
  - 連続空白の1個化
