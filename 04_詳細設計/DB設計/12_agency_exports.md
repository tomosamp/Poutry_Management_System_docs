...
- ALICブロイラーの外部協会値は `payload_json` に含めて保存する。
...
| 10 | payload_json | 提出データJSON | JSON |  | ○ |  | 出力時の値スナップショット（年次は12ヶ月分の系列/提出月/ハイライト対象月を含む）。外部協会値は `external` キーに保持する。 |  |
...
| 2026-01-06 | v0.2 | ALIC外部協会値のpayload保持を追記 | Codex |
